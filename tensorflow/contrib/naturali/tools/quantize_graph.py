...
class GraphRewriter(object):
  ...
  def octize_nodes_recursively(self, current_node):
    ...
    if "Linear/MatMul/Enter" in current_node.name and current_node.op == "Enter" and not "Cell0" in current_node.name:
      new_node = tf.NodeDef()
      new_node.CopyFrom(current_node)
      new_node.name = current_node.name
      set_attr_dtype(new_node, "T", tf.qint8)
      self.add_output_graph_node(new_node)
    elif (current_node.name == "model/linear/linear_weights/read" or ("Linear/Matrix/read" in current_node.name and not "Cell0" in current_node.name)) and current_node.op == "Identity": 
      new_node = tf.NodeDef()
      new_node.CopyFrom(current_node)
      new_node.name = current_node.name
      set_attr_dtype(new_node, "T", tf.qint8)
      self.add_output_graph_node(new_node)
    elif (current_node.name == "model/linear/linear/MatMul" or ("Linear/MatMul" in current_node.name and not "Cell0" in current_node.name)) and current_node.op == "MatMul": 
      new_node = tf.NodeDef()
      new_node.CopyFrom(current_node)
      ...
      s = "Not found"
      m = re.search("Cell\d+/.*/Linear", new_node.name)
      if m:
        s = m.group(0)
      if current_node.name == "model/linear/linear/MatMul":
        s = "model/linear/linear_weights"
      if s != "Not found" and s in self.dict_input:
        inherited_attr = self.dict_input[s]
        set_attr_float(new_node, "scale", inherited_attr[0])
        set_attr_tensor(new_node, "bias", inherited_attr[1], tf.float32)
      self.add_output_graph_node(new_node)
    elif (current_node.name == "model/linear/linear_weights" or ("Linear/Matrix" in current_node.name and not "Cell0" in current_node.name)) and current_node.op == "Const":
      new_node, scale, bias = octize_weight_int8_signed(current_node)
      set_attr_dtype(new_node, "dtype", tf.qint8)
      # model weight
      if current_node.name == "model/linear/linear_weights":
        self.dict_input["model/linear/linear_weights"] = (scale, bias)  
      # Cell Weight Matrix
      m = re.search("Cell\d+\/.*\/Linear", new_node.name)
      if m:
        self.dict_input[m.group(0)] = (scale, bias)
      self.add_output_graph_node(new_node)
    else:
      new_node = tf.NodeDef()
      new_node.CopyFrom(current_node)
